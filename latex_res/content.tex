...
In \Cref{sect:related_problems}, we overview the area of problems where 
this technique proves useful. \Cref{sect:prelims} gives necessary 
...
after addition of monomials. See also \Cref{sect:prelims_other}} 
...
See also \Cref{sect:prelims_algebra}). \omnote{I'm not sure what to put for $\Z$ here, 
...
polynomial to $n$ in time if $k$ is fixed. See \Cref{sect:prelims_other}}. 
...
\Cref{sect:prelims_other} gives a table of other notation and terminology used 
...
at most $k/{2^l}$ \cite{Williams09}. For example with $k$-path, 
...
\Cref{sect:other_improvements} briefly overviews other ideas or 
improvements in utilizing the \emph{general} framework of algebraic fingerprints.
...
the idea of discarding squared elements, and fingerprints are merely 
introduced to prevent multilinear terms from canceling out. 
...
he focused on the coefficients, and used fingerprints to pair non-solution 
monomials together to cancel them. To prevent canceling valid solutions, 
\citeauthor{Björklund14} used the same high-level idea of randomization 
by \citeauthor{KouWil09} to disrupt the pairing of solution monomials by 
making them unique.

For Hamiltonicity, this approach allowed \citeauthor{Björklund14} to utilize 
fast algorithms for computing determinants of matrices, which resulted in an 
...
the general multilinear monomial detection, 
much effort has been directed into matching the 
deterministic runtime to the randomized one, 
although there still is a significant gap between them.

The state-of-the-art deterministic algorithm uses 
the partial differentials of the polynomial, though 
a combinatorial set representation approach has been proven as equivalent.

- constrained multilinear detection (weighted problems?), temporal graphs

On another front, there has been research into 
...
With \emph{constrained} multilinear monomial detection \\cite{TODO}, 
a solution that is otherwise valid would not correspond to a multilinear 
monomial unless it satisfies some extra constraint, i.e., we detect only solutions that satisfy 
the constraint that is a weight or a cost etc. 
with a specific constraint, i.e., a cost or a counter. Moreover, multilinear monomial 
detection has been extended to temporal graphs (constraints).
...
Moreover, as seen in \Cref{sect:cancel_nonsolutions}, 
