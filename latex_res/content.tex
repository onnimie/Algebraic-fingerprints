...
\]
\amnote{added \texttt{\textbackslash left ... \textbackslash right}}
...
For example, an abstract evaluation of a polynomial
\amnote*{you can drop the set braces}{$P \in \Z[\{a,b,c,d\}]$} of the form 
...
\end{align*}
\amnote{(moved trailing $+$ to beginning of line)}
...
\begin{lemmaSZ}
  ...
  \]
  \amnote{use $\Pr$ for probability}%
  \amnote{use \texttt{\textbackslash frac} in display math}%
\end{lemmaSZ}
...
\amnote*[inline,nomargin]{optimal (and cannot be improved upon)}{optimized}. 
However, 
...
The derandomization of the general framework seems hard\amnote*{no comma before \enquote{since}}, since algebraic fingerprinting 
...
\amnote*{doubled word, choose one}{implies proving} superpolynomial 
...
\amnote*{assuming an algorithm that decides existence (I suppose)?}{(assuming existence) }
...
existence 
\amnote*[inline,nomargin]{can be detected efficiently}{is detected}.
...
have potential for \amnote[inline,nomargin]{exploiting} these properties.
...
\amnote{Aha... Side note: computing the permanent in general is very hard!}
\amnote{Maybe add quick note about what permanent is}
...
\amnote{added \texttt{sortcites} option in preamble}
...
\amnote*{Specifically, one line of research that has been particularly fruitful ...}{
In specific, 
...
fastest algorithms for several \amnote*{?}{network scan statistics} problems.
...
\amnote{Avoid passive esp. in long sentences}
\amnote*[inline,nomargin]{The work of \citeauthor{Björklund14}, which came right
...
specific problems faster \amnote[inline,nomargin]{(or proving that no
