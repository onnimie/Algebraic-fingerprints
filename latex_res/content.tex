...
While the matrix permanent is generally very hard to compute, in characteristic 2 it is identical 
to the matrix determinant which is significantly easier to compute. In simple terms, the permanent is 
the determinant where every subtraction is replaced with addition. Note that in characteristic 2 however, 
there is no distinction between these operations. 
...
done on specific settings. In their series of papers \cite{Chen10, Chen11a, Chen11b, Chen13a}, 
...
were also further studied by \textcite{Chen14}.
...
Using these methodologies, the authors showed applications that outperform the previously 
fastest parallel algorithms for e.g. anomaly detection with graph scan statistics.
