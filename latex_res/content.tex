...
This result was quickly improved in \cite{Williams09}, where a \bigOstar{2^k} algorithm was given.\nl
...
every vertex, in \bigOstar{1.657^n} time. Soon enough, for $k$-path, a \bigOstar{1.66^k} algorithm was found \cite{Björklund17}. 
...
degree of multivariate monomial, $\mathcal{O}$, $\Theta$, FPT, $\mathcal{O}$*, determinism \& non-determinism ( or use 'Monte Carlo' c: )
...
In the next section, this thesis quickly overviews dynamic programming and 
...
multilinear monomial detection and, thus, have faster algorithms [ref forgot :p].\nl 
...
Therefore, a dynamic programming algorithm can be designed for the expansion with an additional rule: 
any squared variable can be instantly discarded.\nl

In practise, a dynamic programming algorithm ... 
TODO: explain dynamic programming briefly and how it works with the arithmetic circuit
