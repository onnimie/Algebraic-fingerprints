...
%
\begin{figure}[!h]
  \includegraphics[width=0.5\textwidth, height=5.6cm]{3D_matching_new.png}
  \centering
...
Therefore, scaling exponentially 
...
Thus, for an algorithm to reliably detect a multilinear monomial, 
it is necessary to use different mappings 
from $X$ to $Y$ until a multilinear monomial survives the mapping. 
...
Moreover, it yields a general framework for 
solving combinatorial problems in parameterized complexity 
since the technique focuses on the abstract $k$-multilinear monomial detection, 
to which many combinatorial problems can be reduced to \cite{KouWil15}.
...
which is a methodology that detects anomalies in a graph. 
An anomaly is a small connected subgraph of vertices that maximize some scan statistics function, 
i.e. a function of values associated with a vertex. 
This is relevant in 
...
\bigO{n^3k^n} = \bigOstar{k^n}. Ditto for $\Omega$*.
...
For example, the arithmetic circuit presented in Figure \ref{fig:circuit_example} 
gives an abstract evaluation of a polynomial $P \in \Z[a,b,c,d]$ of the form 
$P = (ab+cd)^2$.  
...
\begin{figure}[h]
  \includegraphics[width=0.8\textwidth, height=6.3cm]{arithmetic_circuit_EXAMPLE.png}
  \centering
...
Therefore, any progress in the general multilinear monomial detection implies 
...
since we have abstracted away the specific algebrization into multilinear monomial detection. 
...
\textcite{Williams09} developed the technique further, 
generalizing the algebra to $GF(2^{l})[\Z_2^k]$. 
...
survive under \emph{any} assignment $X \to \mathbf{F}[\Z_2^k]$. 
...
we notice that the multilinear monomial in $P_2$ has an even coefficient, and thus 
...
and augment them to $P_2$ as follows: 
\begin{align*}
...
could assign every $x \in X \cup S$ to $\mathbf{F}[\Z_2^k]$. 
...
However, introducing new indeterminates raises the degree of monomials. 
...
pick a unique $s_i \in S$. Insert a new (scalar) multiplication gate $\overbar{G_i}$ that takes 
...
\begin{figure}[!h]
  \includegraphics[width=\textwidth, height=7.5cm]{arithmetic_circuit_FINGERPRINTS.png}
  \centering
...
whether the polynomial $P'(\overbar{X}, S) \in \mathbf{K}[S]$ is not identical to $\mathbf{0}$, 
i.e., with $\Phi$ representing the family of mappings $S \to \mathbf{F}$, 
\[
  \exists \phi \in \Phi %, \overbar{S} = \{\phi(s) \: | \: s \in S\} 
  \colon P'(\overbar{X}, \phi) \neq \mathbf{0}.
\]
...
that algebraic fingerprinting and the runtime of \bigOstar{2^k} are in some sense 
...
probably requires a different approach from polynomial identity testing. 
...
In algebraic fingerprinting, however, we do not have this information on the monomials: 
...
(i.e., constructing a \emph{search} algorithm given an oracle that decides the existence of a solution) 
is often easy. 
...
The evaluation of the polynomial is done sequentially in the algebraic fingerprinting framework. 
However, the matrix representations of the group algebras 
...
This yields a randomized algorithm for 
