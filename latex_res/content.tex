...
This implies that \bigO{e^{\abs{Y}}} random mappings must be tried for a multilinear monomial to survive 
with a reasonable constant probability. Thus, a $k$-multilinear monomial is detected with a 
non-deterministic algorithm in \bigOstar{(2e)^{\abs{Y}}} time, where $\abs{Y} \propto k$.
...
can be expressed algebraically: any squared variable should be identical to zero. 
\begin{equation}
...
Assigning values from an algebra $\mathbf{G}$ into $X$ to meet (\ref{eq:squared_to_zero}) 
...
$d$-dimensional matrix group with matrix multiplication. Note that for (\ref{eq:squared_to_zero}), 
\[
...
To prevent multilinear terms from canceling due to characteristic, we 
use a set of auxiliary indeterminates $F$. 
...
This results in the uniqueness of multilinear monomials, which prevents cancelation.
...
$k$-multilinear monomial detection has a lower bound of \bigOstar{2^k}. 
...
Indeed, faster algorithms have been found by designing new algebrizations with techniques 
similar to algebraic fingerprinting, where the fingerprints are designed to 
abuse the underlying combinatorial properties. In \cref{sect:cancel_nonsolutions}, 
...
offer possibilities for parallelization. In \cref{sect:parallelization}, the ideas of 
\citeauthor{Midas19} behind the distributed multilinear monomial detection \cite{Midas19} are discussed.
